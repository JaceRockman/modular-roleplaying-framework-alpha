...
(defn dice-pool-button
  ...
  (button {:style styles/flat-button-style
           ...
           :disabled-style styles/disabled-flat-button-style
           :disabled-text-style styles/disabled-flat-button-text-style
           :on-press #(doall (swap! active? not)
                             (if (not @active?) nil (>evt [:log-dice-roll char-id action-id dice-pool])))
...
(defn header-format
  ...
  [:> rn/Text {:style {:font-size font-size :color color :padding padding}}
