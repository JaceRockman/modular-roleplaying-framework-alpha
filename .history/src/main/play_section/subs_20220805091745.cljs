...
(find-nested {:physical {:quality {:initiation {:name "Coordination", :value 1}, :reaction {:name "Reflexes", :value 1}, :continuation {:name "Endurance0.", :value 1}}, :power {:dominance {:name "Might", :value 6}, :competence {:name "Finesse", :value 6}, :resilience {:name "Fortitude", :value 6}}}} :value)

(reg-sub
 :experience
 (fn [db [_ char-id]]
   (let [character-stats (get-in db [:characters char-id :stats])
         character-skills (apply map #(:qualty %) (vals character-stats))
         character-abilities (apply map #(:power %) (vals character-stats))
         skill-cost (apply + (map #(:value %) (find-nested character-skills :value)))
         ability-cost (divide (apply + (map #(:value %) (find-nested character-abilities :value))) 2)
