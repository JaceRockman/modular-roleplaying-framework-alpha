...
(defn damage-tracker
  [char-id domain]
  [:> rn/View {:style {:flex-direction :row :width 150 :max-height 80 :align-items :center :justify-content :center :border-width 1 :border-color :white :padding-top 10 :margin-left 1}}
   (widgets/incrementor
...
(defn simple-stat
  ...
  (let [stat-name (string/capitalize (name domain))
    ...
    (widgets/flat-button {:key helpers/new-key
                          :style {:height 80 :width 175 :flex-direction :column :padding 5 :border-width 1 :border-color :white}
