...
(defn resource-details
  ...
  [:> rn/View {:key (helpers/new-key)
   ...
   [:> rn/View {:style {:position :absolute :align-self :flex-end :flex-direction :row}}
    ...
    ;;                        :on-press (fn []
    ;;                                    (let [new-id (<sub [:new-resource-id])]
    ;;                                      (do
    ;;                                        (>evt [:copy-resource resource-id new-id])
    ;;                                        (if char-id
    ;;                                          (do (>evt [:add-character-resource char-id new-id])
    ;;                                              (>evt [:set-active-resource char-id new-id]))
    ;;                                          (let [resource-type (<sub [:resource-type resource-id])]
    ;;                                            (>evt [:set-global-active-resource resource-type new-id]))))))})
    (widgets/float-button {:text-style {:margin 0 :padding 0}
                           :text (:duplicate widgets/common-icons)
                           :on-press (fn []
                                       (do
                                         ...
                                         (reset! creating-resource? true)))})
    (widgets/float-button {:text-style {:margin 0 :padding 0}
                           :text (:edit widgets/common-icons)
                           :on-press (fn [] (>evt [:resource-edit-mode-toggle resource-id]))})
    (widgets/float-button {:text-style {:margin 0 :padding 0}
...
(defn edit-resource-section
  ...
  [:> rn/View {:border-color :white :border-width 2 :margin-top 10 :margin-bottom 10 :padding 10}
   [:> rn/View {:style {:flex-direction :row}}
    [:> rn/Text {:style {:color :white}}
     (:name (<sub [:resource-quality resource-id]))]
    (widgets/h-incrementor #(do (>evt [:update-resource-quality-val resource-id dec])
                                (>evt [:update-exp char-id inc])) false
                           #(do (>evt [:update-resource-quality-val resource-id inc])
                                (>evt [:update-exp char-id dec])) false
                           [:> rn/Text {:key (.toString (random-uuid))
                                        :style {:color :white :margin 5}}
                            (let [val (:value (<sub [:resource-quality resource-id]))]
                              (if (= val 0) "--" (str val "d")))])]
   [:> rn/View {:style {:flex-direction :row}}
    [:> rn/Text {:style {:color :white}} (:name (<sub [:resource-power resource-id]))]
    (widgets/h-incrementor #(do (>evt [:update-resource-power-val resource-id dec]) 
                                (>evt [:update-exp char-id inc])) false
                           #(do (>evt [:update-resource-power-val resource-id inc])
                                (>evt [:update-exp char-id dec])) false
                           [:> rn/Text {:key (.toString (random-uuid))
                                        :style {:color :white :margin 5}}
                            (let [val (:value (<sub [:resource-power resource-id]))]
                              (if (= val 0) "--" val))])] 
   [:> rn/View {:style {:flex-direction :row :align-self :flex-end}}
    (widgets/float-button {:text "Save"
                           :on-press #(do (>evt [:resource-edit-mode-toggle resource-id])
                                          (reset! property-index 0)
                                          (reset! creating-resource? false))})]])

(swap! temp-resource #(assoc % :name "Hello"))
@temp-resource

(defn create-resource-section
  []
  (let [char-id 0
        quality-name-path [:benefits :quality :name]
    ...
    ;;               [:equipment :traits :expertise :affiliations :items]))]
     [:> rn/View {:key (.toString (random-uuid))
                  :style {:flex-direction :row}}
      [:> rn/Text {:style {:color :white :font-size 18}} "Title: "]
      [:> rn/TextInput {:style {:background-color :white :padding-left 10 :padding-right 10 :margin-bottom 3 :margin-top 3 :bottom 5}
                        :on-change-text (fn [i] (swap! temp-resource #(assoc % :name i)))
                        :default-value (:name @temp-resource)}]]
    ...
    ;;                      "Quality"
    ;;                      "Quality Alias")
    ;;   (widgets/h-incrementor (fn [] (swap! temp-resource #(update-in % [:benefits :quality :value] dec))) false
    ;;                          (fn [] (swap! temp-resource #(update-in % [:benefits :quality :value] inc))) false
    ;;                          [:> rn/Text {:key (.toString (random-uuid))
    ;;                                       :style {:color :white :margin 5}}
    ;;                           (let [value (get-in @temp-resource [:benefits :quality :value])]
    ;;                             (if (= value 0) "--" (str value "d")))])]
    ;;  [:> rn/View {:style {:flex-direction :row}}
    ;;   (widgets/text-area (fn [i] (swap! temp-resource #(assoc-in % power-name-path i)))
    ;;                      "Power"
    ;;                      "Power Alias")
    ;;   (widgets/h-incrementor (fn [] (swap! temp-resource #(update-in % [:benefits :power :value] dec))) false
    ;;                          (fn [] (swap! temp-resource #(update-in % [:benefits :power :value] inc))) false
    ;;                          [:> rn/Text {:key (.toString (random-uuid))
    ;;                                       :style {:color :white :margin 5}}
    ;;                           (let [value (get-in @temp-resource [:benefits :power :value])]
    ...
    ;;                     "Details")
     [:> rn/View {:style {:flex-direction :row :align-self :flex-end}}
      (widgets/float-button {:disabled? (not (:type @temp-resource))
                             :text "Save"
                             :on-press #(do (if char-id
                                              (>evt [:create-new-character-resource-2 char-id @temp-resource])
                                              (>evt [:create-new-resource-2 @temp-resource]))
...
;;   [:> rn/View {:border-color :white :border-width 2 :margin-top 10 :margin-bottom 10 :padding 10}
;;    [:> rn/View {:flex-direction :row}
;;     (doall (map (fn [type-val] (widgets/flat-button {:text (helpers/title-case (name type-val))
;;                                                      :active? (= type-val (<sub [:resource-type temp-resource]))
;;                                                      :on-press (fn [] (>evt [:update-resource-type temp-resource type]))}))
;;                 [:equipment :traits :expertise :affiliations :items]))]
;;    (widgets/text-area (fn [i] (>evt [:update-resource-title temp-resource i]))
;;                       "Resource Name"
;;                       "Title")
;;    [:> rn/View {:style {:flex-direction :row}}
;;     (widgets/text-area (fn [i] (>evt [:update-resource-quality-alias temp-resource i]))
;;                        "Quality"
;;                        "Quality Alias")
;;     (widgets/h-incrementor #(>evt [:update-resource-quality-val temp-resource dec]) false
;;                            #(>evt [:update-resource-quality-val temp-resource inc]) false
;;                            [:> rn/Text {:key (.toString (random-uuid))
;;                                         :style {:color :white :margin 5}}
;;                             (let [val (:value (<sub [:resource-quality temp-resource]))]
;;                               (if (= val 0) "--" (str val "d")))])]
;;    [:> rn/View {:style {:flex-direction :row}}
;;     (widgets/text-area (fn [i] (>evt [:update-resource-power-alias temp-resource i]))
;;                        "Power"
;;                        "Power Alias")
;;     (widgets/h-incrementor #(>evt [:update-resource-power-val temp-resource dec]) false
;;                            #(>evt [:update-resource-power-val temp-resource inc]) false
;;                            [:> rn/Text {:key (.toString (random-uuid))
;;                                         :style {:color :white :margin 5}}
;;                             (let [val (:value (<sub [:resource-power temp-resource]))]
;;                               (if (= val 0) "--" val))])]
;;    (widgets/text-area (fn [i] (>evt [:update-resource-details temp-resource i]))
;;                       "Resource Details"
;;                       "Details")
;;    [:> rn/View {:style {:flex-direction :row :align-self :flex-end}}
;;     (widgets/float-button {:disabled? (not (<sub [:resource-type temp-resource]))
;;                            :text "Save"
;;                            :on-press #(do (if char-id
;;                                             (>evt [:create-new-character-resource char-id])
;;                                             (>evt [:create-new-resource]))
;;                                           (>evt [:resource-edit-mode-toggle resource-id])
;;                                           (reset! property-index 0)
;;                                           (reset! creating-resource? false))})
;;     (widgets/float-button {:text "Cancel"
;;                            :on-press #(do (>evt [:reset-temp-resource])
;;                                           (reset! creating-resource? false)
;;                                           (reset! property-index 0))})]])

(def create-resource-button
  (widgets/float-button {:text "+"
                         :on-press #(swap! creating-resource? not)}))
  ...
  ([resource-id char-id edit?]
   (let [active? (some #(= resource-id %) (<sub [:character-resources char-id]))]
     (widgets/flat-button {:style (resource-grid-def 0 {:padding 0 :margin 0})
                           :active? active?
                           :active-style {:padding 0 :margin 0}
                           :on-press #(>evt [:add-character-resource char-id resource-id])
                           :text (<sub [:resource-name resource-id])}))))
  ...
  (let [action-context? (not (= resource-button-fn resource-button))
    ...
    [:> rn/View {:key (helpers/new-key)
     ...
     (if (and (active-row? resource-id char-id edit?) (not action-context?))
       (if (<sub [:resource-edit-mode? resource-id])
         (edit-resource-section char-id resource-id)
...
(defn resource-section
  ...
  (let [resources (if (or (not (or char-id edit?)) (and char-id edit?))
...
(defn resources
  ...
  [:> rn/View {:style {:padding 15 :width "100%"}}
   [:> rn/View {:style {:flex-direction :row}}
    ...
    (if (not @creating-resource?)
      create-resource-button)
    helpers/spacer
    (if char-id [:> rn/View {:top 15 :right 100} (character-experience char-id)])]
  ...
  (into [:> rn/View {:style {:width "95%" :justify-content :center :align-items :center}}]
        (map card
             [(if (<sub [:edit-mode])
                ...
                (if (< 0 (apply + (map count (vals (dissoc (<sub [:character-profile 0]) :name)))))
