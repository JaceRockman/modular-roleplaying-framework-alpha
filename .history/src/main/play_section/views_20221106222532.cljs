  ...
  (let [quality-name-path [:benefits :quality :name]
    ...
    [:> rn/View {:border-color :white :border-width 2 :margin-top 10 :margin-bottom 10 :padding 10}
     [:> rn/View {:flex-direction :row}
      (doall (map (fn [type-val] (widgets/flat-button {:key (helpers/new-key) 
                                                       :text (helpers/title-case (clojure.core/name type-val))
...
(defn profile-line-edit
  [char-id label]
  [:> rn/View {:key (helpers/new-key) :style {:flex-direction :row}}
...
(defn name-edit
  [char-id]
  [:> rn/TextInput {:key (helpers/new-key) 
                    :style {:font-size 24 :background-color :white :padding 6 :margin-top 30 :margin-bottom 30}
...
(defn character-profile
  [char-id]
  [:> rn/View {:key (helpers/new-key)
               :style {:justify-content :center :flex-direction :row :margin-bottom 15}}
