...
(defn resource-details
  ...
  [:> rn/View {:style {:padding 5 :background-color "#444444" :width "100%"}}
...
(defn edit-resource-section
  [char-id resource-id] 
  (let [editable? (<sub [:resource-editable resource-id])
...
(defn resource-row
  ...
  (let [action-context? (not (= resource-button-fn resource-button))
    ...
    [:> rn/View {:style {:display (if (and action-context? action-resource?) :none :flex)}}
...
(defn profile-line-edit
  ...
  [:> rn/View {:style {:flex-direction :row}}
   ...
   [:> rn/TextInput {:style {:background-color :white :padding-left 10 :padding-right 10 :margin-top 3 :margin-botton 3}
...
(defn simple-stat
  ...
  (let [stat-name (string/capitalize (name domain))
    ...
    (widgets/flat-button {:style {:height 100 :width 175 :flex-direction :column :justify-content "center"
...
(defn action-resource-details
  ...
  (let [action-id (<sub [:active-action char-id])
    ...
    [:> rn/View {:style {:align-items :center :flex-direction :row}}
...
(defn pool-display
  [splinter]
  [:> rn/View {:style {:border-width 1 :border-color :white :margin 5}}
    ...
    [:> rn/View {:style {:align-items :center :justify-content :center :padding-top 20}}
     ...
     [:> rn/View {:style {:flex-direction :row :align-items :center :justify-content :center :margin 10}}
      ...
      [:> rn/View {:style {:align-items :center :justify-content :center :max-width "85%" :flex-direction :row :flex-wrap :wrap}}
       (doall (map pool-display splinters))]
      (widgets/flat-button {:style {:height 40 :width 40}
                            :text-style {:bottom 3}
                            :text "+"
...
(defn dice-roll-display
  ...
  [:> rn/View {:flex-direction :row}
   ...
   (doall (map #(conj [:> rn/Text {:style {:color :white}}] (str % " ")) roll))
...
(defn roll-results
  [char-id action-id]
  [:> rn/View {:style {:padding 20 :align-items :center :justify-content :center}}
   (widgets/header-format {:font-size 24 :color :white} "Roll Results")
   (if (> 2 @active-action-tab)
     [:> rn/Text {:style {:color :white}}
      "Passive Result: " (helpers/passive-result (<sub [:simple-action-roll-value char-id 100 @active-action-tab {}]))])
