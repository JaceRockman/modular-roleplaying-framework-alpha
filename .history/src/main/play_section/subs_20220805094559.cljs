...
;; (({:name Coordination, :value 1} {:name Reflexes, :value 1} {:name Endurance, :value 1}) ({:name Exertion, :value 1} {:name Instinct, :value 1} {:name Perseverance, :value 1}) ({:name Concentration, :value 1} {:name Recognition, :value 1} {:name Comprehension, :value 1}) ({:name Persuasion, :value 1} {:name Insight, :value 1} {:name Connections, :value 1}))
...
(reg-sub
 ...
 (fn [db [_ char-id]]
   (let [character-stats (get-in db [:characters char-id :stats])
         ...
         skill-cost (apply + (map #(:value %) (flatten (map #(find-nested % :value) character-skills))))
         ability-cost (divide (apply + (map #(:value %) (flatten (map #(find-nested % :value) character-abilities)))) 2)
         stat-cost (+ skill-cost ability-cost)
         character-resources (keys (get-in db [:characters char-id :resources]))
         calc-resource-cost (fn [resource-id] (let [resource-benefits (get-in db [:resources resource-id :benefits])
                                                    resource-quality (get-in resource-benefits [:quality :value])
                                                    resource-power (get-in resource-benefits [:quality :value])]
                                                (+ resource-quality resource-power)))
         resource-cost (apply + (map calc-resource-cost character-resources))]
     (+ stat-cost resource-cost))))
