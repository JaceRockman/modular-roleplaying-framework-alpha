...
(reg-event-db
 :create-new-resource
 (fn [db [_ new-id resource-details]]
   (assoc-in db [:resources new-id] (assoc resource-details :id new-id))))

(reg-event-db
 :set-resource-in-edit-mode
 (fn [db [_ resource-id]]
   (assoc db :resource-in-edit-mode resource-id)))

(reg-event-db
 :set-all-resource-vals
 (fn [db [_ resource-id resource-vals]]
   (assoc-in db [:resources resource-id] resource-vals)))
...
(reg-event-db
 :add-character-resource-with-limit
 (fn [db [_ char-id resource-id resource-limit]]
   (let [resource-type (get-in db [:resources resource-id :type])
         character-resources (keys (get-in db [:characters char-id :resources]))
         character-resource-types (frequencies (map #(get-in db [:resources % :type]) character-resources))
         at-limit? (<= resource-limit (get character-resource-types resource-type))]
     (println character-resource-types)
     (assoc-in db [:characters char-id :resources resource-id] {:notes ""}))))
...
(reg-event-db
 :set-temp-resource-vals
 (fn [db [_ temp-resource resource-id]]
   (reset! temp-resource (get-in db [:resources resource-id]))))
