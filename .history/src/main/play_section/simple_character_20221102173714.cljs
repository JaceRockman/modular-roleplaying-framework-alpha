...
(defn simple-stat-row
  ...
  [:> rn/View {:key (helpers/new-key)
   ...
   (if damage?
     ...
     [:> rn/View {:width 100}])
   [:> rn/View {:style {:height 90 :overflow :visible :flex-direction :row}}
    [:> rn/ScrollView {:style {:horizontal true :overflow :visible :flex-direction :row :border-width 0 :margin-top -3
                              :content-container-style {:width 5000 :flex-direction :row}}} (doall (map resource (repeat char-id) (repeat domain) (<sub [(resource-subs domain) char-id])))]]])
