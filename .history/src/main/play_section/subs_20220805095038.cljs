...
(defn calc-resource-cost 
  [db resource-id] 
  (let [resource-benefits (get-in db [:resources resource-id :benefits])
        resource-quality (get-in resource-benefits [:quality :value])
        resource-power (get-in resource-benefits [:quality :value])]
    (println resource-benefits)
...
(reg-sub
 ...
 (fn [db [_ char-id]]
   (let [character-stats (get-in db [:characters char-id :stats])
         ...
         resource-cost (apply + (map #(calc-resource-cost % db) character-resources))]
     (println (calc-resource-cost 1 db))
