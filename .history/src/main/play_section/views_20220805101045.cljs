    ...
    [:> rn/View {:border-color :white :border-width 2 :margin-top 10 :margin-bottom 10 :padding 10}
     ...
     [:> rn/View {:style {:flex-direction :row}}
      (widgets/text-area (fn [i] (swap! temp-resource #(assoc-in % quality-name-path i)))
                         ...
                         "Quality Alias")
      (widgets/h-incrementor (fn [] (swap! temp-resource #(update-in % quality-name-path dec)))
                             (> -3 (get-in @temp-resource [:benefits :quality :value]))
                             (fn [] (swap! temp-resource #(update-in % quality-name-path inc))) false
                             [:> rn/Text {:key (.toString (random-uuid))
                                          :style {:color :white :margin 5}}
                              (let [val (get-in @temp-resource quality-name-path)]
                                (if (= val 0) "--" (str val "d")))])]
     [:> rn/View {:style {:flex-direction :row}}
      (widgets/text-area (fn [i] (swap! temp-resource #(assoc-in % power-name-path i)))
                         (get-in @temp-resource power-name-path)
                         "Power Alias")
      (widgets/h-incrementor (fn [] (swap! temp-resource #(update-in % power-name-path dec))) false
                             (fn [] (swap! temp-resource #(update-in % power-name-path inc))) false
                             [:> rn/Text {:key (.toString (random-uuid))
                                          :style {:color :white :margin 5}}
                              (let [val (get-in @temp-resource power-name-path)]
                                (if (= val 0) "--" (str val)))])]
