...
(defn simple-stat
  ...
  (let [stat-name (string/capitalize (name domain))
        ...
        skill-value (int (apply #(/ (apply + %) 3) (map #(:value (<sub [:character-stat char-id %])) skill-paths)))
        ability-value (int (apply #(/ (apply + %) 3) (map #(:value (<sub [:character-stat char-id %])) ability-paths)))]
                          ...
                          :text [:> rn/View {:style {:flex-direction :row}}
                                 [:> rn/View {:style {:flex-direction :column :padding 0}}
                                  (widgets/flat-button {:style {:display (if (<sub [:edit-mode]) "flex" "none") :width 25 :height 25 :padding 0}
                                                        :on-press (map #(do (>evt [:increase-skill char-id %])
                                                                       (>evt [:update-exp char-id dec])) skill-paths)
                                                        :text-style {:font-size 10 :height 25 :top 7}
                                                        :disabled? (> 1 (<sub [:experience char-id]))
                                                        :text (:up-sm widgets/common-icons)})
                                  (widgets/flat-button {:style {:display (if (<sub [:edit-mode]) "flex" "none") :width 25 :height 25 :padding 0}
                                                        :on-press (map #(do (>evt [:decrease-skill char-id %])
                                                                       (>evt [:update-exp char-id inc])) skill-paths)
                                                        :text-style {:font-size 10 :height 25 :top 7}
                                 ...
                                 [:> rn/View {:style {:flex-direction :column}}
                                  (widgets/flat-button {:style {:display (if (<sub [:edit-mode]) "flex" "none") :width 25 :height 25 :padding 0}
                                                        :on-press (map #(do (>evt [:increase-ability char-id %])
                                                                       (>evt [:update-exp char-id dec])) ability-paths)
                                                        :disabled? (> 1 (<sub [:experience char-id]))
                                                        :text-style {:font-size 10 :height 25 :top 7}
                                                        :text (:up-sm widgets/common-icons)})
                                  (widgets/flat-button {:style {:display (if (<sub [:edit-mode]) "flex" "none") :width 25 :height 25 :padding 0}
                                                        :on-press (map #(do (>evt [:decrease-ability char-id %])
