  ...
  (let [resource-name     (<sub [:resource-name resource-id])
    ...
    [:> rn/View {:key (helpers/new-key)
     ...
     (widgets/flat-button {:style {:height 90 :width 175 :flex-direction :column :border-width 1 :border-color :white}
                           ...
                           :active? (some #(= resource-id %) (<sub [:active-resources 0]))
...
(defn damage-tracker
  [char-id domain]
  [:> rn/View {:key (helpers/new-key)
               :style {:flex-direction :row :width 150 :height 90 :align-items :center :justify-content :center :border-width 1 :border-color :white :padding-top 10 :margin-left 1}}
   (widgets/incrementor
   ...
   [:> rn/Text {:key (helpers/new-key)
                :style {:color (if (<sub [:wounded? char-id domain]) :red :white) :text-align :center :left 5}} (str "Total:\n" (<sub [:char-total-damage char-id domain]))]])
...
(defn character-stats-and-resources
  ...
  [:> rn/View {:style {:width "100%" :padding-bottom 15}}
   ...
   (doall (map simple-stat-row
               ...
               (repeat simple-stat)
               (repeat true)))
   [:> rn/View {:style {:flex-wrap :wrap :margin-top -1 :flex-direction :row}}
...
(defn play-section
  ...
  (case (<sub [:active-tab 1])
    ...
    :else [:> rn/Text {:key (helpers/new-key) :style {:color :white}} "Invalid Tab"]))