...
(defn simple-stat-row
  ...
  [:> rn/View {:key (helpers/new-key)
               :style {:scroll-enabled true :flex-direction :row :align-items :center}}
   (domain domain-icons)
   (stat-fn char-id domain)
   (if damage?
     (damage-tracker char-id domain)
     [:> rn/View {:width 100}])
   [:> rn/View {:style {:height 95 :width 360 :horizontal true :border-width 0 :margin-top -1 :flex-direction :row :overflow :scroll}} 
