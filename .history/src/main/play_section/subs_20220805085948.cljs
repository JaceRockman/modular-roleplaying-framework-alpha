...
(defn find-nested
  [m k]
  (->> (tree-seq map? vals m)
       (filter (fn [node] (some #(= :name %) (keys node))))))

(filter map? (tree-seq map? vals {:physical {:quality {:initiation {:name "Coordination", :value 1}, :reaction {:name "Reflexes", :value 1}, :continuation {:name "Endurance", :value 1}}, :power {:dominance {:name "Might", :value 6}, :competence {:name "Finesse", :value 6}, :resilience {:name "Fortitude", :value 6}}}}))

(reg-sub
 :experience
 (fn [db [_ char-id]]
   (let [character-stats (get-in db [:characters char-id :stats])
         stat-cost (find-nested character-stats #(get % "value"))]
