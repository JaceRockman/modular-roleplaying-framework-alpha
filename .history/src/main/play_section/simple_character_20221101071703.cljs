...
(defn simple-stat
  ...
  (let [stat-name (string/capitalize (name domain))
    ...
    (widgets/flat-button {:key helpers/new-key
                          :style {:height 100 :width 175 :flex-direction :column :padding 5 :border-width 1 :border-color :white}
                          ...
                          :text [:> rn/View {:style {:flex-direction :row}}
                                 [:> rn/View {:style {:flex-direction :column :padding 0}}
                                  (widgets/flat-button {:style {:display (if (<sub [:edit-mode]) "flex" "none") :width 25 :height 20 :padding 0 :justify-content :center}
                                                        :on-press (fn []
                                  ...
                                  (widgets/flat-button {:style {:display (if (<sub [:edit-mode]) "flex" "none") :width 25 :height 20 :padding 0}
                                                        :on-press (fn []
...
(defn simple-stat-row
  ...
  [:> rn/View {:key (helpers/new-key)
   ...
   (if damage?
     (main-play-section/damage-tracker char-id domain)
     [:> rn/View {:width 100}])])
...
(defn character-stats
  ...
  [:> rn/View {:style {:width "100%" :padding-bottom 15}}
   ...
   (doall (map simple-stat-row
               ...
               (repeat main-play-section/simple-stat)
               (repeat true)))])

(defn character-data
  [char-id]
  [:> rn/View
   (character-stats char-id "simple")
   (widgets/button {})])
