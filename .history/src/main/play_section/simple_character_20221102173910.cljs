...
(defn simple-stat-row
  ...
  [:> rn/View {:key (helpers/new-key)
   ...
   (if damage?
     ...
     [:> rn/View {:width 100}])
   [:> rn/ScrollView {:style {:height 90 :width 5000 :horizontal true :overflow :visible :flex-direction :row :border-width 0 :margin-top -5
                              :content-container-style {:width 5000 :flex-direction :row :overflow :visible}}} (doall (map resource (repeat char-id) (repeat domain) (<sub [(resource-subs domain) char-id])))]])
