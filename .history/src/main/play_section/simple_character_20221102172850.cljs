...
(defn simple-stat-row
  ...
  [:> rn/View {:key (helpers/new-key)
   ...
   (if damage?
     ...
     [:> rn/View {:width 100}])
   [:> rn/ScrollView {:style {:horizontal true :height 90 :overflow :hidden :nested-scroll-enabled true}} (doall (map resource (repeat char-id) (repeat domain) (<sub [(resource-subs domain) char-id])))]])
