...
(defn incrementor
  ...
  [:> rn/View {:style {:flex-direction :column :align-items :center :justify-content :center}}
...
(defn h-incrementor
  ...
  [:> rn/View {:style {:flex-direction :row :align-items :center :justify-content :center :bottom 5}}
   (flat-button {:style {:width 30 :height 30 :padding 0}
                 :on-press dec-fn
                 :text-style {:font-size 10}
...
(defn text-section
  ([headers paragraphs]
   (into [] (reduce (fn [view [header paragraph]]
                    ...
                    [:> rn/View]
                    (doall (map vector headers paragraphs)))))
                          ...
                          (concat [(header-format {:font-size 24 :color :white} header)]
                                  ...
                                  (let [display-example? (r/atom true)
                                    ...
                                    (if (some? example)
                                      ...
                                      [[:> rn/View {:style {:padding-bottom 20}}]])))))
                    [:> rn/View]
                    (doall (map vector headers paragraphs examples))))))

(defn text-area
  [evt sub label]
  [:> rn/View {:style {:flex-direction :row}}
   [:> rn/Text {:style {:color :white :font-size 18}} (str label ": ")]
   [:> rn/TextInput {:style {:max-length 10000 :background-color :white :max-width "90%" :padding-left 10 :padding-right 10 :margin-bottom 3 :margin-top 3 :bottom 5}
                     :multiline true
                     :on-change-text #(evt %)
                     :default-value sub}
    ""]])

(defn text-area-single-line
  [evt sub label]
  [:> rn/View {:style {:flex-direction :row}}
   [:> rn/Text {:style {:color :white :font-size 18}} (str label ": ")]
   [:> rn/TextInput {:style {:max-length 10000 :background-color :white :max-width "90%" :padding-left 10 :padding-right 10 :margin-bottom 3 :margin-top 3 :bottom 5}
...
(defn text-submission
  ...
  [:> rn/View {:flex-direction :row}
   [:> rn/Text {:style {:color :white :font-size 18}} (str label ": ")]
   [:> rn/TextInput {:style {:max-length 10000 :background-color :white :padding-left 10 :padding-right 10 :margin-bottom 3 :margin-top 3 :bottom 5}
                     :on-change-text #(evt %)
                     :default-value sub}
    ""]])
